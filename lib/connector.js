      ...
      res
        ...
        .on('end', () => {
          if (res.statusCode !== 200) {
            const err = new Error(`dS Response ${res.statusCode}: ${data}`);
          ...
          if (!json.ok) {
            const err = new Error(`dS Response ${res.statusCode}: ${json.message}`);
            err.status = res.statusCode;
            return next(err);
          }
...
module.exports = (path, param, next) => {
