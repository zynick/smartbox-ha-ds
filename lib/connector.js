...
const stackHttpQuery = (href, next) => {
  // query
  const symbol = href.indexOf('?') === -1 ? '?' : '&';
  const url = `https://${DS_HOST}:${DS_PORT}${href}${symbol}token=${sessionToken}`;
  _httpsGet(url, next);
};
